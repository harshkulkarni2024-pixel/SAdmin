...
export const incrementUsage = async (userId: number, type: 'story' | 'image' | 'chat'): Promise<void> => {
    ...
    if (type === 'story') { 
        ...
        action = `یک سناریوی استوری (${user.story_requests + 1}/2 روزانه) تولید کرد.`;
    }
    else if (type === 'image') {
        columnToIncrement = 'image_requests';
        action = `یک تصویر (${user.image_requests + 1}/35 هفتگی) تولید کرد.`;
    }
    else if (type === 'chat') {
        columnToIncrement = 'chat_messages';
        action = `یک پیام در چت (${user.chat_messages + 1}/150 هفتگی) ارسال کرد.`;
    }

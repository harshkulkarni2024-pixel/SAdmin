...
export const incrementUsage = async (userId: number, type: 'story' | 'chat' | 'caption_idea'): Promise<void> => {
    ...
    if (type === 'story') { 
        ...
        action = `یک کپشن از ایده (${user.caption_idea_requests + 1}/${user.caption_idea_limit ?? 5} روزانه) تولید کرد.`;
    }
    else if (type === 'chat') {
...
ALTER TABLE public.users ADD COLUMN caption_idea_requests INTEGER DEFAULT 0 NOT NULL;
ALTER TABLE public.users ADD COLUMN caption_idea_limit INTEGER DEFAULT 5 NOT NULL;
...
-- SQL for Algorithm News Feature (NEW SCHEMA - SUPPORTS HISTORY)
-- 1. Drop the old table if it exists
DROP TABLE IF EXISTS public.algorithm_news;
-- 2. Create the new table
CREATE TABLE public.algorithm_news (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  content TEXT NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW() -- Keep updated_at for potential future edits
);
...
-- 5. (Optional) Insert a first message if the table is empty
INSERT INTO public.algorithm_news (content) VALUES ('هنوز خبری ثبت نشده است. لطفاً از پنل مدیریت اولین خبر را وارد کنید.');
*/
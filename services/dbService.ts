import type { User, PostScenario, Plan, Report, Caption, PostIdea, BroadcastMessage, ActivityLog, ChatMessage, SubscriptionHistory, AlgorithmNews } from '../types';
...
export const saveChatHistory = async (userId: number, messages: ChatMessage[]): Promise<void> => {
    ...
    const cleanMessages = messages.map(({imageUrl, isInterim, ...rest}) => rest);
...
// --- Algorithm News ---
export const getAlgorithmNews = async (): Promise<AlgorithmNews | null> => {
    if (!supabase) throw new Error(SUPABASE_INIT_ERROR);
    const { data, error } = await supabase.from('algorithm_news').select('*').limit(1).single();
    if (error) {
        handleError(error, 'getAlgorithmNews');
        throw new Error(`خطای پایگاه داده: ${error.message}`);
    }
    return data;
};

export const updateAlgorithmNews = async (content: string): Promise<void> => {
    if (!supabase) throw new Error(SUPABASE_INIT_ERROR);
    ...
    if (error) {
        handleError(error, 'updateAlgorithmNews');
        throw new Error(`خطای پایگاه داده: ${error.message}`);
    }
};
...
CREATE TABLE public.subscription_history (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id BIGINT REFERENCES public.users(user_id) ON DELETE CASCADE, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), extended_for_days INT NOT NULL, new_expiry_date TIMESTAMPTZ NOT NULL);
ALTER TABLE public.subscription_history ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Allow authenticated users to read their own history" ON public.subscription_history FOR SELECT TO authenticated USING (auth.uid() = (SELECT user_uid FROM users WHERE user_id = subscription_history.user_id));
...
-- SQL for Algorithm News Feature (NEW)
CREATE TABLE public.algorithm_news (id BIGINT PRIMARY KEY, content TEXT NOT NULL, updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
INSERT INTO public.algorithm_news (id, content) VALUES (1, 'هنوز خبری ثبت نشده است. لطفاً از پنل مدیریت اولین خبر را وارد کنید.');
...
CREATE POLICY "Allow admin (service_role) to update" ON public.algorithm_news FOR UPDATE TO service_role USING (true);
*/
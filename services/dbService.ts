...
export const updateUserInfo = async (userId: number, info: { about_info: string, preferred_name: string }): Promise<void> => {
    ...
    if (error) {
        if (error.message.includes('column "preferred_name" of relation "users" does not exist')) {
             throw new Error(`خطای پایگاه داده: ستون 'preferred_name' در جدول 'users' پیدا نشد.

لطفاً اسکریپت SQL راهنمای موجود در انتهای فایل 'services/dbService.ts' را اجرا کنید تا ستون‌های مورد نیاز به جدول کاربران اضافه شوند.`);
        }
        handleError(error, 'updateUserInfo');
        throw new Error(`خطا در ذخیره اطلاعات: ${error.message}`);
    }
};

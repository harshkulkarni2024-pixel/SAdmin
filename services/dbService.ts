...
const handleError = (error: any, context: string) => {
    // Don't log if supabase is not initialized, as this is an expected state.
...
export const verifyAccessCode = async (code: string, isSessionLogin: boolean = false): Promise<User | null> => {
    // --- MOCK USER LOGIC ---
    // Handle login via access code for mock users, bypassing Supabase.
    if (!isSessionLogin) {
        if (code === ADMIN_ACCESS_CODE) return Promise.resolve(MOCK_ADMIN_USER);
        if (code === TEST_USER_ACCESS_CODE) return Promise.resolve(MOCK_TEST_USER);
    }
    // Handle login via session (user ID) for mock users, bypassing Supabase.
    if (isSessionLogin) {
        const userId = parseInt(code, 10);
        if (userId === ADMIN_IDS[0]) return Promise.resolve(MOCK_ADMIN_USER);
        if (userId === TEST_USER_ID) return Promise.resolve(MOCK_TEST_USER);
    }
    // --- END MOCK USER LOGIC ---

    // For real users (including M2 and T2), proceed with Supabase.
    // If the codes are not M1 or T1, they will fall through to this database logic.
    if (!supabase) return null;
    let query;
    ...
    if (error && error.code !== 'PGRST116') { // Ignore "no rows found" but throw for other DB errors
...
export const getAllUsers = async (): Promise<User[]> => {
    const { data, error } = await supabase?.from('users').select('*').neq('user_id', ADMIN_IDS[0]) || { data: [], error: null };
...
export const getUserById = async (userId: number): Promise<User | null> => {
    // --- MOCK USER LOGIC ---
    // Return mock users directly to avoid unnecessary database calls for them.
    if (userId === ADMIN_IDS[0]) return Promise.resolve(MOCK_ADMIN_USER);
    if (userId === TEST_USER_ID) return Promise.resolve(MOCK_TEST_USER);
    // --- END MOCK USER LOGIC ---

    const { data, error } = await supabase?.from('users').select('*').eq('user_id', userId).single() || { data: null, error: null };
...
export const addUser = async (fullName: string, accessCode: string): Promise<{ success: boolean, message: string }> => {
    if (!supabase) return { success: false, message: 'اتصال به پایگاه داده برقرار نیست.' };
...
export const deleteUser = async (userId: number): Promise<void> => {
    const { error } = await supabase?.from('users').delete().eq('user_id', userId) || { error: null };
...
export const updateUserAbout = async (userId: number, about: string): Promise<void> => {
    const { error } = await supabase?.from('users').update({ about_info: about }).eq('user_id', userId) || { error: null };
...
export const incrementUsage = async (userId: number, type: 'story' | 'image' | 'chat'): Promise<void> => {
    ...
    if (type === 'story') { 
        ...
        action = `یک سناریوی استوری (${user.story_requests + 1}/2) تولید کرد.`;
    }
    else if (type === 'image') {
...
export const getPlanForUser = async (userId: number): Promise<Plan | null> => {
    const { data, error } = await supabase?.from('plans').select('*').eq('user_id', userId).maybeSingle() || { data: null, error: null };
    if (error) handleError(error, 'getPlanForUser');
    return data;
};

export const savePlanForUser = async (userId: number, content: string): Promise<void> => {
    const { error } = await supabase?.from('plans').upsert({ user_id: userId, content, timestamp: new Date().toISOString() }, { onConflict: 'user_id' }) || { error: null };
    if (error) handleError(error, 'savePlanForUser');
};

export const deletePlanForUser = async (userId: number): Promise<void> => {
    const { error } = await supabase?.from('plans').delete().eq('user_id', userId) || { error: null };
    if (error) handleError(error, 'deletePlanForUser');
};

// --- Reports ---
export const getReportsForUser = async (userId: number): Promise<Report[]> => {
    const { data, error } = await supabase?.from('reports').select('*').eq('user_id', userId).order('timestamp', { ascending: false }) || { data: [], error: null };
...
export const saveReportForUser = async (userId: number, content: string): Promise<void> => {
    const { error } = await supabase?.from('reports').insert({ user_id: userId, content, timestamp: new Date().toISOString() }) || { error: null };
    if (error) handleError(error, 'saveReportForUser');
};

export const deleteReportForUser = async (userId: number): Promise<void> => {
    const { data, error } = await supabase?.from('reports').delete().eq('user_id', userId) || { data: null, error: null };
    if (error) handleError(error, 'deleteReportForUser');
};

// --- Scenarios ---
export const getScenariosForUser = async (userId: number): Promise<PostScenario[]> => {
    const { data, error } = await supabase?.from('scenarios').select('*').eq('user_id', userId).order('scenario_number', { ascending: true }) || { data: [], error: null };
    if (error) handleError(error, 'getScenariosForUser');
    return data || [];
};
export const getScenarioById = async (id: number): Promise<PostScenario | null> => {
    const { data, error } = await supabase?.from('scenarios').select('*').eq('id', id).single() || { data: null, error: null };
...
export const addScenarioForUser = async (userId: number, scenarioNumber: number, content: string): Promise<void> => {
    const { error } = await supabase?.from('scenarios').insert({ user_id: userId, scenario_number: scenarioNumber, content }) || { error: null };
    if (error) handleError(error, 'addScenarioForUser');
};
export const deleteScenario = async (scenarioId: number): Promise<void> => {
    const { error } = await supabase?.from('scenarios').delete().eq('id', scenarioId) || { error: null };
    if (error) handleError(error, 'deleteScenario');
};

// --- Ideas ---
export const getIdeasForUser = async (userId: number): Promise<PostIdea[]> => {
    const { data, error } = await supabase?.from('ideas').select('*').eq('user_id', userId) || { data: [], error: null };
...
export const deleteIdea = async (ideaId: number): Promise<void> => {
    const { error } = await supabase?.from('ideas').delete().eq('id', ideaId) || { error: null };
    if (error) handleError(error, 'deleteIdea');
};

// --- Captions ---
export const getCaptionsForUser = async (userId: number): Promise<Caption[]> => {
    const { data, error } = await supabase?.from('captions').select('*').eq('user_id', userId).order('created_at', { ascending: false }) || { data: [], error: null };
...
export const addCaption = async (userId: number, title: string, content: string, originalScenarioContent: string): Promise<void> => {
    const { error } = await supabase?.from('captions').insert({ user_id: userId, title, content, original_scenario_content: originalScenarioContent }) || { error: null };
    if (error) handleError(error, 'addCaption');
};

// --- Broadcasts ---
export const getLatestBroadcast = async (): Promise<BroadcastMessage | null> => {
    const { data, error } = await supabase?.from('broadcasts').select('*').order('timestamp', { ascending: false }).limit(1).single() || { data: null, error: null };
...
export const addBroadcast = async (message: string): Promise<void> => {
    const { error } = await supabase?.from('broadcasts').insert({ message, timestamp: new Date().toISOString() }) || { error: null };
...
export const getActivityLogs = async (): Promise<ActivityLog[]> => {
    const { data, error } = await supabase?.from('activity_logs').select('*').order('created_at', { ascending: false }).limit(100) || { data: [], error: null };
...
export const getChatHistory = async (userId: number): Promise<ChatMessage[]> => {
    const { data, error } = await supabase?.from('chat_history').select('messages').eq('user_id', userId).single() || { data: null, error: null };
...
export const saveChatHistory = async (userId: number, messages: ChatMessage[]): Promise<void> => {
    const { error } = await supabase?.from('chat_history').upsert({ user_id: userId, messages }, { onConflict: 'user_id' }) || { error: null };
...
export const getStoryHistory = async (userId: number): Promise<{ id: number; content: string }[]> => {
    const { data, error } = await supabase?.from('story_history').select('stories').eq('user_id', userId).single() || { data: null, error: null };
...
export const getImageHistory = async (userId: number): Promise<{ id: number; url: string }[]> => {
    const { data, error } = await supabase?.from('image_history').select('images').eq('user_id', userId).single() || { data: null, error: null };
...
export const getNotificationCounts = async (userId: number): Promise<{ scenarios: number, plans: number, reports: number }> => {
    const { data: scenarios, error: sError } = await supabase?.from('scenarios').select('id', { count: 'exact' }).eq('user_id', userId) || { data: [], error: null };
...
export const getAdminNotificationCounts = async (): Promise<{ ideas: number, logs: number }> => {
    const { data: ideas, error: iError } = await supabase?.from('ideas').select('id', { count: 'exact' }) || { data: [], error: null };
    ...
    const { count: logsCount, error: lError } = await supabase?.from('activity_logs').select('*', { count: 'exact', head: true }).gt('created_at', lastLogTime) || { count: 0, error: null };

...
const getAiClient = (): GoogleGenAI => {
    ...
    const apiKey = import.meta.env?.VITE_API_KEY || 'AIzaSyDkU_EJdZeNWmJxLMhJoKP9s-wJfoynAKc';
    if (!apiKey) {
...
export const startChatSession = (userAbout: string, history: FormattedChatMessage[]): Chat => {
    ...
    const systemInstruction = `
        ...
        پاسخ‌هایت باید ظاهری زیبا و خوانا داشته باشند. بین پاراگراف‌ها فاصله بگذار تا خواندن متن راحت‌تر شود.
    `;
    try {

...
export const startChatSession = (user: User, history: {role: 'user' | 'model', parts: {text: string}[]}[]): Chat => {
    const client = getAiClient();
    const greetingName = user.preferred_name?.trim() || user.full_name;
    ...
    const systemInstruction = `
        ...
        **Formatting Rules:**
        ...
        You are assisting ${user.full_name}. In conversations, you must address them by their preferred name: **${greetingName}**. 
        
        Here is some information about the user's business/page which you should use for content strategy, but not for addressing them:
        ---
        ${user.about_info}
        ---

...
export const startChatSession = (userAbout: string, history: FormattedChatMessage[]): Chat => {
    ...
    const systemInstruction = `
        ...
        کاربر را به تولید محتوای بیشتر و فعالیت جدی در اینستاگرام تشویق کن و به او انگیزه بده.
        
        **قوانین قالب‌بندی:**
        - از اموجی‌ها به طور مناسب برای زیبایی و انتقال حس استفاده کن. 🥳
        - برای تاکید روی کلمات کلیدی، آن‌ها را بین تگ‌های <b> و </b> قرار بده. مثلا: <b>این خیلی مهمه!</b>
        - برای خوانایی بهتر، بین پاراگراف‌ها یک خط فاصله بگذار. (از <p> یا دو <br> استفاده کن)
...
export const getLatestAlgorithmNews = async (): Promise<{ text: string, groundingChunks: any[] | undefined }> => {
    try {
        const client = getAiClient();
        const prompt = "به عنوان یک متخصص رسانه‌های اجتماعی، با استفاده از جستجوی گوگل، آخرین تغییرات و اخبار الگوریتم اینستاگرام در هفته گذشته را پیدا کن. نتایج را به صورت یک لیست از موارد کلیدی ارائه بده. برای هر مورد، یک عنوان (تیتر) کوتاه و جذاب همراه با یک اموجی مرتبط بنویس و آن را داخل تگ <h3> قرار بده. سپس در پاراگراف بعدی، آن را به صورت کاربردی و با لحنی ساده برای تولیدکنندگان محتوا توضیح بده. از اموجی‌ها در متن توضیحات هم برای جذابیت بیشتر استفاده کن. بین هر خبر و خبر بعدی یک خط خالی فاصله بگذار. از هرگونه کلمه یا عبارت اضافی مثل مقدمه یا نتیجه‌گیری خودداری کن و فقط لیست اخبار را ارائه بده. از کاراکتر * یا # استفاده نکن.";

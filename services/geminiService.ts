...
export async function* generateStoryScenarioStream(userAbout: string, goal: string, idea: string, yesterdayFeedback: string): AsyncGenerator<string> {
  try {
    ...
    **دستورالعمل‌ها:**
    ...
    6.  کلمات کلیدی و مهم را با تگ <b> پررنگ کن. از # و * برای لیست‌ها استفاده نکن.
...
export const generateCompetitorAnalysis = async (competitors: string[]): Promise<{text: string, groundingChunks: any[]}> => {
    try {
        const client = getAiClient();
        const competitorList = competitors.map(c => `@${c}`).join(', ');
        const prompt = `
        به عنوان یک تحلیلگر ارشد شبکه‌های اجتماعی، یک گزارش خلاصه و کلی از فعالیت‌های اخیر پیج‌های اینستاگرام ${competitorList} ارائه بده.
        این گزارش باید ترکیبی از تحلیل تمام رقبا باشد و نه جداگانه.
        
        مواردی که باید بررسی شوند:
        - **موضوعات محتوایی اصلی:** اخیراً روی چه موضوعاتی تمرکز کرده‌اند؟
        - **استراتژی‌های تعامل:** از چه روش‌هایی برای درگیر کردن مخاطب استفاده می‌کنند؟ (مثلاً مسابقه، پرسش و پاسخ، محتوای چالشی)
        - **فرمت‌های محتوایی:** بیشتر از ریلز، پست اسلایدی یا استوری استفاده می‌کنند؟
        - **روندهای جدید:** آیا روند یا استراتژی جدیدی در محتوای آن‌ها مشاهده می‌شود؟
        
        گزارش باید به زبان فارسی، کاملاً واضح، کاربردی و با فرمت‌بندی مناسب (استفاده از سرفصل، بولت‌پوینت و متن پررنگ) باشد.
        `;

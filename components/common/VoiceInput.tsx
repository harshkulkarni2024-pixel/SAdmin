...
export const VoiceInput: React.FC<VoiceInputProps> = ({ onTranscriptChange, currentValue, disabled = false }) => {
  ...
  useEffect(() => {
    // Prevent this effect from running on the initial render and clearing the input.
    if (!isMounted.current) {
      isMounted.current = true;
      return;
    }
    
    // The hook provides the complete transcript for the current listening session.
    // We combine it with the text that existed before listening started.

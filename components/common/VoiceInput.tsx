...
export const VoiceInput: React.FC<VoiceInputProps> = ({ onTranscriptChange, currentValue, disabled = false }) => {
  ...
  // Effect to append transcript when recognition ends naturally (e.g., on pause or manual stop)
  const memoizedOnTranscriptChange = useCallback(onTranscriptChange, []);
  useEffect(() => {
    if (!isListening && transcript) {
      memoizedOnTranscriptChange((currentValue.trim() ? currentValue + ' ' : '') + transcript);
    }
  }, [isListening, transcript, currentValue, memoizedOnTranscriptChange]);

...
type AdminViewType = 'users' | 'broadcast' | 'activity' | 'vip_management';
...
const AdminView: React.FC<AdminViewProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    const handlePopState = async (event: PopStateEvent) => {
        ...
        } else if (state?.adminView && ['users', 'broadcast', 'activity', 'vip_management'].includes(state.adminView)) {
            setSelectedUser(null);
    ...
    const initializeView = async () => {
        ...
        } else if (['broadcast', 'activity', 'vip_management'].includes(hash)) {
  ...
  const renderContent = () => {
    ...
    switch (activeView) {
      case 'users':
        return <UserManagement key={userListVersion} onSelectUser={handleSelectUser} onLogout={onLogout} />;
      case 'vip_management':

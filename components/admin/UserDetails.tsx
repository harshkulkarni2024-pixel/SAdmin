...
type DetailTab = 'about' | 'plans' | 'reports' | 'scenarios' | 'ideas' | 'limits';
...
const UserDetails: React.FC<UserDetailsProps> = ({ user, onBack, onUpdate }) => {
    ...
    const refreshData = useCallback(async (updatedUser?: User) => {
        ...
        const userPlan = await db.getPlanForUser(userToFetch.user_id);
        setPlan(userPlan?.content || '');

        const userReports = await db.getReportsForUser(userToFetch.user_id);
        setReport(userReports.map(r => `تاریخ: ${new Date(r.timestamp).toLocaleDateString('fa-IR')}\n${r.content}`).join('\n\n---\n\n') || '');

        const userScenarios = await db.getScenariosForUser(userToFetch.user_id);
        setScenarios(userScenarios);

        const userIdeas = await db.getIdeasForUser(userToFetch.user_id);
        setIdeas(userIdeas);

        setLimits({
            story_requests: userToFetch.story_requests,
            caption_idea_requests: userToFetch.caption_idea_requests,
            image_requests: userToFetch.image_requests,
            chat_messages: userToFetch.chat_messages,
        });
    ...
    const handleSave = async () => {
     ...
     const handleSaveLimits = async () => {
        try {
            await db.updateUserUsageLimits(user.user_id, limits);
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'limits':
                 return (
                    <div>
                        <p className="text-slate-400 text-sm mb-4">میزان مصرف فعلی کاربر را مشاهده و در صورت نیاز مقادیر را ویرایش کنید. تغییرات بلافاصله اعمال می‌شوند.</p>
                        {notification && (
                        ...
                        <div className="space-y-4">
                             {Object.entries({
                                story_requests: 'تولید استوری (روزانه)',
                                caption_idea_requests: 'تولید کپشن (روزانه)',
                                image_requests: 'تولید عکس (هفتگی)',
                                chat_messages: 'پیام‌های چت (هفتگی)',
                            }).map(([key, label]) => (
                                 <div key={key} className="flex items-center justify-between">
                                    ...
                                    <input
                                        ...
                                        className="w-24 bg-slate-900 p-2 rounded text-center"
                                    />
                                </div>
                            ))}
                        </div>

...
const UserDetails: React.FC<UserDetailsProps> = ({ user, onBack, onUpdate }) => {
    ...
    const handleSaveAbout = async () => {
        try {
            ...
            showNotification(`خطا در ذخیره اطلاعات: ${(e as Error).message}`);
        }
    };
    ...
    const handleSaveLimits = async () => {
        try {
            ...
            showNotification(`خطا در ذخیره محدودیت‌ها: ${(error as Error).message}`);
        }
    };
                    ...
                    <div>
                        <div className="bg-slate-900/70 p-4 rounded-lg mb-6">
                            ...
                            <div className="relative">
                                <textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} className="w-full h-32 bg-slate-700 p-2 ps-4 pe-4 rounded mb-2" placeholder={`محتوای ${title} جدید را اینجا بنویسید...`}></textarea>
                            </div>
                    ...
                    <div>
                        <div className="bg-slate-900/70 p-4 rounded-lg mb-6">
                            ...
                            <input type="number" placeholder="شماره سناریو" value={newScenario.number} onChange={e => setNewScenario({...newScenario, number: e.target.value})} className="w-full bg-slate-700 p-2 rounded mb-2" />
                             <div className="relative">
                                <textarea placeholder="محتوای سناریو" value={newScenario.content} onChange={e => setNewScenario({...newScenario, content: e.target.value})} className="w-full h-24 bg-slate-700 p-2 ps-4 pe-4 rounded mb-2"></textarea>
                             </div>
    ...
    return (
        <div className="animate-fade-in">
            ...
            <h1 className="text-3xl font-bold text-white mb-4">مدیریت کاربر: {currentUser.full_name}</h1>
             ...
             {notification && (
                <div className={`p-3 mb-4 rounded-lg text-sm transition-opacity duration-300 ${notification.includes('خطا') ? 'bg-red-900/50 text-red-300' : 'bg-green-900/50 text-green-300'}`}>
                    {notification}
                </div>
            )}

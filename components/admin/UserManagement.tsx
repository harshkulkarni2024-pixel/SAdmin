...
const UserManagement: React.FC<UserManagementProps> = ({ onSelectUser, onLogout }) => {
  ...
  const handleAddUser = async () => {
      ...
      if (result.message === 'اتصال به پایگاه داده برقرار نیست.') {
          finalMessage = `اتصال به پایگاه داده برقرار نیست.
این خطا معمولا زمانی رخ می‌دهد که برنامه با کاربر نمایشی (M1) اجرا شده و هنوز به پایگاه داده واقعی متصل نشده است.
لطفاً مطمئن شوید که متغیرهای VITE_SUPABASE_URL و VITE_SUPABASE_ANON_KEY در فایل .env به درستی تنظیم شده‌اند.`;
      }
      
      setNotification({ type: result.success ? 'success' : 'error', message: finalMessage });

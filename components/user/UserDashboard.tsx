...
const DashboardCard: React.FC<{
  ...
  >
    ...
    <div className="flex flex-col items-start">
        ...
        {isVip && <span className="mt-1 text-xs bg-violet-600 text-white font-bold px-1.5 py-0.5 rounded-md shadow-lg shadow-violet-500/50 animate-pulse">VIP</span>}
    </div>
  </button>
);


const UserDashboard: React.FC<UserDashboardProps> = ({ setActiveView }) => {
  ...
  const refreshDashboard = useCallback(async () => {
    ...
    if(user.is_vip) {
        ...
        if (plans.length > 0) {
            const lastPlanViewTime = Number(localStorage.getItem(`lastView_plans_${user.user_id}`) || 0);
            const newPlans = plans.filter(p => new Date(p.timestamp).getTime() > lastPlanViewTime);
            if(newPlans.length > 0) {
                 allNews.push({id: `plan_${newPlans[0].id}`, type: 'برنامه جدید', content: `شما ${newPlans.length} برنامه محتوایی جدید دارید.`, date: newPlans[0].timestamp});
            }
        }
  ...
  const handleNewsClick = useCallback((item: NewsItem) => {
    if (!user) return;
    // Hide the item from the UI immediately.
    ...
    setDismissedNews(prevDismissed => {
        const newDismissed = [...prevDismissed, item.id];
        // Save the updated list to localStorage.
        localStorage.setItem(`dismissedNews_${user.user_id}`, JSON.stringify(newDismissed));
        return newDismissed;
    });
    
    // Navigate if the news item is clickable.
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-4">
          ...
          <DashboardCard title="گفتگوی زنده" icon="phone-wave" onClick={() => handleCardClick('live_chat')} />
        ...
        <div className="mt-12">
            ...
            <div className="space-y-3">
                {otherNewsItems.map((item) => {
                    const isClickable = !!NEWS_NAVIGATION_MAP[item.type];
                    return (
                        <div 
                            key={item.id}

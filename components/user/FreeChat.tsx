...
// Define a local type for the parts of a chat message to improve type safety.
...
const FreeChat: React.FC<FreeChatProps> = ({ user, onUserUpdate }) => {
    ...
    const initializeChat = async (history: ChatMessage[]) => {
        try {
            ...
            const session = startChatSession(user, formattedHistory);
            setChat(session);
            setMessages(history);
    ...
    const handleSend = async () => {
        ...
        try {
            const promptParts: ChatPart[] = [{ text: currentInput }];
            if (currentImage) {
                promptParts.push({
                    inlineData: {
    ...
    const handleClearHistory = async () => {
        if (window.confirm('آیا مطمئن هستید که می‌خواهید تمام تاریخچه گفتگو را پاک کنید؟ این عمل غیرقابل بازگشت است.')) {
            try {
                ...
                setMessages([]);
                initializeChat([]); // Re-initialize chat session with empty history
            } catch (error) {
    ...
    return (
        <div className="h-full flex flex-col max-w-4xl mx-auto animate-fade-in">
            ...
            {user.is_vip && (
                 <a href="#live_chat" className="mb-4 flex items-center justify-center gap-2 w-full p-3 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-colors animate-pulse shadow-lg shadow-violet-500/50">
                     <Icon name="chat" className="w-5 h-5 text-violet-400" />
                ...
                {uploadedImage && (
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    <div className="relative flex-1">
                        ...
                        <VoiceInput onTranscriptChange={setInput} currentValue={input} disabled={isLoading || remainingMessages <= 0 || !chat} />
                    </div>
                 </div>
            </div>
        </div>

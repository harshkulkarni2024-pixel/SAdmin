...
export type UserViewType = 'dashboard' | 'story' | 'post_scenario' | 'plans' | 'reports' | 'image_video' | 'caption' | 'chat' | 'live_chat' | 'post_idea' | 'algorithm_news' | 'competitor_analysis';

const VALID_VIEWS: UserViewType[] = ['dashboard', 'story', 'post_scenario', 'plans', 'reports', 'image_video', 'caption', 'chat', 'live_chat', 'post_idea', 'algorithm_news', 'competitor_analysis'];
...
const UserView: React.FC<UserViewProps> = ({ user, onLogout, onUserUpdate }) => {
  ...
  useEffect(() => {
    ...
    let initialView = VALID_VIEWS.find(v => v === initialHash) || 'dashboard';
    if (VIP_VIEWS.includes(initialView) && !user.is_vip) {
       ...
       <header className="flex-shrink-0 bg-slate-900/80 backdrop-blur-lg flex items-center h-20 px-6 border-b border-slate-800 relative">
          ...
          <div className="flex-1 text-center">
           ...
           <div className="absolute left-6 top-1/2 -translate-y-1/2">
                <button onClick={() => setIsSubscriptionModalOpen(true)} className="p-2 text-yellow-400 hover:text-yellow-300 relative" title="وضعیت اشتراک">

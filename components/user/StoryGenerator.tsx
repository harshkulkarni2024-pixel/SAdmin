                                ...
                                {expandedHistoryId === item.id && (
                                     <div className="p-4 border-t border-slate-700 space-y-4">
                                        {item.content.split('---').map((part, index) => {
                                            ...
                                            if (!trimmedPart) return null;
                                            return (
                                                 <div key={index} className="bg-slate-900/50 p-3 rounded-md prose prose-sm prose-invert prose-p:text-slate-300 prose-strong:text-white whitespace-pre-wrap" dangerouslySetInnerHTML={{ __html: trimmedPart }} />
                                            )
                                        })}
                                    </div>
                                )}

...
const parseScenario = (content: string): StorySlide[] => {
    const slides: StorySlide[] = [];
    const parts = content.split('---').filter(p => p.trim());

    parts.forEach(part => {
        const titleMatch = part.match(/عنوان:\s*(.*)/);
        const instructionMatch = part.match(/دستورالعمل:\s*([\s\S]*?)(?=متن_روی_استوری:|$)/);
        const storyTextMatch = part.match(/متن_روی_استوری:\s*([\s\S]*)/);
        
        slides.push({
            title: titleMatch ? titleMatch[1].trim() : '',
            instruction: instructionMatch ? instructionMatch[1].trim() : '',
            storyText: storyTextMatch ? storyTextMatch[1].trim() : '',
        });
    });
    
    return slides.filter(s => s.title || s.instruction || s.storyText);
};
...
const StoryGenerator: React.FC<StoryGeneratorProps> = ({ user, onUserUpdate }) => {
    ...
    const storySlides = parseScenario(currentScenario);

    const renderScenarioSlides = (content: string) => {
        const slides = parseScenario(content);
        return slides.map((slide, index) => (
             <div key={index} className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 space-y-3">
                {slide.title && <h3 className="text-md font-bold text-white bg-violet-800/50 p-2 rounded-md inline-block">{slide.title.replace(/استوری \d+:/, (match) => `${match.trim()}`)}</h3>}
                {slide.instruction && (
                     <div>
                 ...
                 {slide.storyText && (
                    <div className="border-t border-slate-700 pt-3">
                        ...
                        <p className="text-white whitespace-pre-wrap mt-1">{slide.storyText}</p>
                    </div>
                )}
            </div>
        ));
    ...
    return (
        <div className="max-w-3xl mx-auto animate-fade-in">
                        ...
                        <p className="text-xs text-slate-400 mb-2">مثلاً: آنباکسینگ محصول جدید، یک روز از زندگی من، یا پاسخ به سوالات مخاطبان...</p>
                         <div className="relative">
                            ...
                            <textarea
                                ...
                                className="w-full h-32 p-4 pe-36 bg-slate-800 border border-slate-700 rounded-lg text-white placeholder-slate-500 focus:ring-2 focus:ring-violet-500 focus:outline-none resize-none"
                                disabled={isLoading}
                            />
            ...
            {isLoading && storySlides.length === 0 && (
                             ...
                             <div key={item.id} className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
                                ...
                                {expandedHistoryId === item.id && (
                                     <div className="p-4 border-t border-slate-700 space-y-4">
                                        {renderScenarioSlides(item.content)}
                                    </div>
                                )}
                            </div>
                        ))}

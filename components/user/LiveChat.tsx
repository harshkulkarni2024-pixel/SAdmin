...
const LiveChat: React.FC<LiveChatProps> = ({ user }) => {
    ...
    // Use a ref to hold the latest version of the stopConversation function
    const stopConversationRef = useRef(stopConversation);
    useEffect(() => {
        stopConversationRef.current = stopConversation;
    });

    // Effect to handle cleanup on component unmount, now with a stable reference
    useEffect(() => {
        return () => {
    ...
    const startConversation = async () => {
        ...
        try {
            ...
            const userName = sanitize(user.preferred_name || user.full_name);
            const userBio = sanitize(user.about_info || 'اطلاعاتی ثبت نشده');
            // A simpler, more robust instruction to avoid potential parsing issues on the server side (like WebSocket error 1011).
            const systemInstruction = `You are "Item", a friendly AI assistant. Your user is an Instagram content creator named ${userName}. Their bio is: "${userBio}". You must speak Persian. Start the conversation with a brief, friendly greeting to ${userName}.`;
            ...
            const sessionPromise = ai.live.connect({
                ...
                callbacks: {
                    ...
                    onclose: (e: CloseEvent) => {
                        console.warn('Live chat connection closed:', e);

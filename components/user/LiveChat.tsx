...
const LiveChat: React.FC = () => {
    ...
    const speak = useCallback((text: string) => {
        ...
        const findAndSetVoice = () => {
            ...
            if (voices.length === 0) {
                // This can happen on some browsers, wait for the event.
                return;
            }
            const persianVoice = voices.find(v => v.lang.toLowerCase().startsWith('fa'));
            
            if (persianVoice) {
                utterance.voice = persianVoice;
            } else {
                if (!sessionStorage.getItem('noPersianVoiceNotified')) {
                    showNotification('صدای فارسی برای پخش یافت نشد. ممکن است پاسخ‌ها با صدای انگلیسی پخش شوند. لطفاً از نصب بودن صدای فارسی در تنظیمات سیستم عامل خود اطمینان حاصل کنید.', 'info');
                    sessionStorage.setItem('noPersianVoiceNotified', 'true');
                }
            }
            utterance.lang = 'fa-IR'; // Set language regardless
        ...
        if (!isListening && transcript.trim() && status === 'listening') {
            ...
            generateChatResponseStream(user, newHistory, { text: userMessage.text }, (chunk) => {
                ...
                setHistory(prev => {
                    ...
                    // This case handles the very first chunk creating the message
            ...
            .catch(err => {
                ...
                const formattedError = handleAiError(err);
                showNotification(formattedError, 'error');
                setHistory(prev => [...prev, { sender: 'ai', text: `خطا: ${formattedError}` }]);
                setStatus('idle');
            });
        }
    }, [isListening, transcript, status, user, history, showNotification, speak]);
    ...
    return (
        <div className="h-full flex flex-col max-w-4xl mx-auto animate-fade-in">
            <div className="text-center mb-4">
                ...
                <p className="text-slate-500 text-xs mt-1">برای بهترین نتیجه، واضح و در محیطی آرام صحبت کنید.</p>
            </div>

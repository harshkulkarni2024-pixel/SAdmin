...
const LiveChat: React.FC<LiveChatProps> = ({ user }) => {
    ...
    const stopConversation = useCallback(async (isUnmounting = false) => {
        if (sessionPromiseRef.current) {
            sessionPromiseRef.current.then(session => session.close()).catch(() => {});
        }

        if (hasSavedOnUnmount.current) {
             cleanup();
             if(!isUnmounting) setConnectionState('idle');
             return;
        }
        hasSavedOnUnmount.current = true;

        const finalUserMsg = currentInputTranscription.current.trim();
        const finalAiMsg = currentOutputTranscription.current.trim();
        
        let finalTranscripts = [...transcriptsRef.current];
        if (finalUserMsg) finalTranscripts.push({ sender: 'user', text: finalUserMsg });
        if (finalAiMsg) finalTranscripts.push({ sender: 'ai', text: finalAiMsg });

        const validTranscripts = finalTranscripts.filter(t => t.text.trim() !== '');
    ...
    const startConversation = async () => {
        ...
        try {
            ...
            // ROOT CAUSE FIX for 1011 Error: Use a static, simple system instruction.
            // Dynamic, user-provided strings can contain characters that break the WebSocket connection.
            const systemInstruction = 'You are "Item", a friendly AI expert in Instagram content strategy. You must speak Persian. Start by warmly greeting the user.';
            ...
            const sessionPromise = ai.live.connect({
                ...
                callbacks: {
                    ...
                    onerror: (e: ErrorEvent) => {
                        console.error('Live chat error event:', e);
                        setError(`خطا در ارتباط: ${e.message || 'Unknown error'}`);
                        setConnectionState('error');
                        cleanup();
                    },
                    onclose: (e: CloseEvent) => {
                        if (e.code !== 1000) { // 1000 is a normal closure
                ...
                {connectionState === 'connecting' && (
                 ...
                 {connectionState === 'connected' && (
                    <div className="text-center">
                        <button onClick={() => stopConversation(false)} className="w-full px-6 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 transition-colors">
                            پایان و ذخیره گفتگو
                        </button>
                    </div>
                )}

                ...
                callbacks: {
                    ...
                    onmessage: async (message: LiveServerMessage) => {
                        if (message.serverContent) {
                            ...
                            if (inputTranscription?.text) {
                                inProgressTranscriptsRef.current.user = inputTranscription.text;
                                setInProgressTranscript(prev => ({ ...prev, user: inputTranscription.text ?? '' }));
                            }
                            if (outputTranscription?.text) {
                                const newText = outputTranscription.text;
                            ...
                            if (audioData) {
                                const outputCtx = audioResources.current.outputCtx;
                                if(outputCtx){
                            ...
                            if (interrupted) {
                                const res = audioResources.current;
                                for (const source of res.playingSources.values()) {
                                    source.stop();
                                    res.playingSources.delete(source);
                                }
                                nextAudioStartTime.current = 0;
                            }
                        }
                    ...
                    onclose: (e: CloseEvent) => {
                        if (e.code !== 1000) { // 1000 = normal closure
                          let detailedError = `اتصال به صورت غیرمنتظره قطع شد. کد: ${e.code}. لطفا دوباره تلاش کنید.`;
                          if (e.code === 1011) {
                              detailedError += "\n\nاین خطا معمولاً به یک مشکل در سمت سرور اشاره دارد. لطفاً موارد زیر را بررسی کنید:\n۱. مطمئن شوید قابلیت پرداخت (Billing) برای پروژه Google Cloud شما فعال است.\n۲. چند دقیقه صبر کرده و دوباره تلاش کنید؛ ممکن است مشکل موقتی باشد.";
                          }

...
const LiveChat: React.FC<LiveChatProps> = ({ user }) => {
    ...
    // Ref to prevent multiple saves on cleanup
    ...
    const cleanup = useCallback(() => {
     ...
     const stopConversation = useCallback(async () => {
        ...
        if (transcripts.length > 0 && !hasSavedOnUnmount.current) {
    ...
    }, [user.user_id, transcripts, cleanup]);
    ...
    const startConversation = async () => {
        ...
        try {
            ...
            const sessionPromise = ai.live.connect({
                model: 'gemini-25-flash-native-audio-preview-09-2025',
                callbacks: {
                    onopen: () => {
                ...
                {connectionState === 'connecting' && (
                 ...
                 {connectionState === 'connected' && (
                    <div className="text-center">
                        <button onClick={stopConversation} className="w-full px-6 py-3 bg-violet-600 text-white font-semibold rounded-lg hover:bg-violet-700 transition-colors">
                            پایان و ذخیره گفتگو
                        </button>
                    </div>
                )}

...
const SubscriptionStatus: React.FC<SubscriptionStatusProps> = ({ onClose }) => {
    const { user } = useUser();
    ...
    const handleRequestNotificationPermission = () => {
        if (!('Notification' in window)) {
            showNotification('این مرورگر از نوتیفیکیشن پشتیبانی نمی‌کند.', 'error');
            return;
        }

        if (Notification.permission === 'granted') {
            showNotification('شما قبلاً دسترسی به نوتیفیکیشن را داده‌اید.', 'info');
            return;
        }
        ...
        if (Notification.permission !== 'denied') {
            Notification.requestPermission().then(permission => {
                if (permission === 'granted') {
                    showNotification('عالی! نوتیفیکیشن‌ها فعال شدند.', 'success');
                } else {
                    showNotification('شما دسترسی به نوتیفیکیشن را رد کردید.', 'info');
                }
            });
        } else {
            showNotification('شما دسترسی به نوتیفیکیشن را مسدود کرده‌اید. لطفاً از تنظیمات مرورگر آن را فعال کنید.', 'error');
        }
    };

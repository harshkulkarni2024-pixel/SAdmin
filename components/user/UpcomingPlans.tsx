...
// FIX: Changed getPlanForUser to getPlansForUser as it is the correct exported function.
...
const UpcomingPlans: React.FC<UpcomingPlansProps> = ({ user, onUserUpdate }) => {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
        // FIX: Called getPlansForUser which returns an array, and took the first (most recent) plan.
        const userPlans = await getPlansForUser(user.user_id);
        setPlan(userPlans[0] ?? null);
    };
    fetchPlan();

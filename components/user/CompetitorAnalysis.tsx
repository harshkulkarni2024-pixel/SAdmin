                ...
                {history.length > 0 && (
                     <div className="mt-12">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            {history.map(item => (
                                <button key={item.id} onClick={() => setAnalysisResult(item)} className="bg-slate-800 p-2 rounded-lg aspect-square flex flex-col items-center justify-center text-center hover:bg-slate-700 transition-colors">
                                    <img src={item.imageUrl} alt="تحلیل قبلی" className="w-full h-full object-cover rounded" />
                                </button>
                            ))}
                        </div>
                    </div>
